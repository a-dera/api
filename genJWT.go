...
var (
	host                = "https://gateway-ksdxotlq6a-uc.a.run.app"               //flag.String("host", "", "The API host. Required.")
	audience            = "https://gateway-ksdxotlq6a-uc.a.run.app"               //flag.String("audience", "", "The audience for the JWT. equired")
	serviceAccountFile  = "gateway_service_key.json"                              //flag.String("service-account-file", "", "Path to service account JSON file. Required.")
	serviceAccountEmail = "dunia-gateway@business-282516.iam.gserviceaccount.com" //flag.String("service-account-email", "", "Path email associated with the service account. Required.")
)

func main() {
	//flag.Parse()

	jwt, err := generateJWT(serviceAccountFile, serviceAccountEmail, audience, 3600000)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println(jwt)
